...
pub(crate) fn handle_completion(
    ...
    // salsa storage keeps thread-local query state. `snapshot()` hands
    // the reader a `WorldStateSnapshot`, so the background thread can query oak
    // but can't call a setter.
