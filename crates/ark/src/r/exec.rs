...
impl RFunction {
    ...
    fn call_impl(&mut self, protect: &mut RProtect) -> SEXP {
        unsafe {
            ...
            if !self.package.is_empty() {
                lhs = self
                    .protect
                    .add(Rf_lang3(rsymbol!(":::"), rsymbol!(self.package), lhs));
            }
            ...
            let try_catch = self.protect.add(Rf_lang3(
                rsymbol!("::"),
                rsymbol!("base"),
                rsymbol!("tryCatch"),
            ));
            let call = self
                .protect
                .add(Rf_lang3(try_catch, call, rsymbol!("identity")));
            SET_TAG(call, R_NilValue);
...
mod tests {
    ...
    fn test_basic_function() {
        unsafe {
            ...
            let result = RFunction::from("+").add(2).add(2).call(&mut protect);
