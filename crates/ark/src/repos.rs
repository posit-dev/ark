...
#[cfg(target_os = "linux")]
fn get_ppm_linux_repo(repo_url: Option<url::Url>, linux_name: String) -> String {
    let generic_url = match repo_url {
        Some(url) => url,
        None => url::Url::parse(GENERIC_P3M_REPO).unwrap(),
    };
    if linux_name.is_empty() {
        return generic_url.to_string();
    }
...
fn get_ppm_binary_package_repo(repo_url: Option<url::Url>) -> String {
    #[cfg(target_os = "linux")]
    {
        ...
        get_ppm_linux_repo(
            repo_url,
            get_p3m_linux_codename(id, version, version_codename),
        )
    }
...
mod tests {
    ...
    fn test_get_p3m_linux_repo() {
        ...
        for (distro, expected) in test_cases {
            let result = get_ppm_linux_repo(None, distro.to_string());
    ...
    fn test_get_custom_ppm_linux_repo() {
        ...
        for (distro, expected) in test_cases {
            let result = get_ppm_linux_repo(Some(custom_url.clone()), distro.to_string());
            assert_eq!(result, expected);
        }
    }
}
