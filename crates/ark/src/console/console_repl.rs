...
//! REPL implementation for the R console.
//!
...
pub(super) struct ActiveReadConsoleRequest {
    exec_count: u32,
    request: ExecuteRequest,
    pub(super) originator: Originator,
...
#[derive(Default)]
pub(super) enum ReadConsolePendingAction {
...
// LSP inputs generated by `ReadConsole`.
...
/// Returns global-environment names and one scope for each parent environment.
pub(crate) fn console_scopes() -> Vec<Vec<String>> {
    // TODO: Include the selected debug environment when debugging.
    // https://github.com/posit-dev/positron/issues/3001
    let env = Environment::new(R_ENVS.global.into());
    env.ancestors().map(|env| env.names()).collect()
}
