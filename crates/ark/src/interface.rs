...
impl RMain {
    ...
    fn read_console(
        ...
        // If debugger is active, we need to step automatically by returning `n`
        // to R in two cases:
        ...
        if self.debug_is_debugging {
            ...
            let mut at_injected_breakpoint = false;
            if let DebugCallText::Finalized(ref text, DebugCallTextKind::DebugAt) =
                self.debug_call_text
            {
                at_injected_breakpoint = text.contains(".ark_breakpoint");
            }
