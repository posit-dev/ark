...
impl RMain {
    ...
    unsafe fn eval(expr: libr::SEXP, srcref: libr::SEXP, buf: *mut c_uchar, buflen: c_int) {
        let env = RMain::get().read_console_env();

        // SAFETY: This may jump in case of error, keep this POD
        unsafe {
            let frame = libr::Rf_protect(env.into());
...
pub extern "C-unwind" fn r_read_console(
    ...
    let env: RObject = match main.read_console_pending_action.take() {
        ReadConsolePendingAction::None => {
            ...
            if main.needs_browser_capture(prompt) {
                main.read_console_pending_action
                ...
                RMain::on_console_input(buf, buflen, input).unwrap();
                return 1;
            }
    ...
    main.do_read_console_entry(env);
