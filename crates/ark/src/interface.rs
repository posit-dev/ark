...
/// ## Safety of mutation
///
/// `RMain` is meant to be used as a singleton by a single thread, the R thread.
/// We store it in a thread-local variable in a `RefCell`. It's preferred that
/// the methods on `RMain` do not take `&mut self` because they are callbacks
/// called for various events by R. Borrowing mutably from the RefCell could
/// cause a panic if a method is unexpectedly called while another method is
/// running.
pub struct RMain {
    ...
    autoprint_output: RefCell<String>,
...
impl RMain {
    ...
    fn init_execute_request(&mut self, req: &ExecuteRequest) -> (ConsoleInput, u32) {
        ...
        self.autoprint_output.replace(String::new());
    ...
    fn make_execute_reply(
        ...
        // Safety: We don't hold long-lived RefCell references to `autoprint_output`.
        let mut autoprint = self.autoprint_output.replace(String::new());
    ...
    fn write_console(&self, buf: *const c_char, _buflen: i32, otype: i32) {
        ...
        if stream == Stream::Stdout && is_auto_printing() {
            ...
            if self.pending_lines.is_empty() {
                // Safety: We don't hold long-lived RefCell references to `autoprint_output`.
                self.autoprint_output.borrow_mut().push_str(&content);
                return;
            }
...
pub extern "C" fn r_write_console(buf: *const c_char, buflen: i32, otype: i32) {
    RMain::with(|main| main.write_console(buf, buflen, otype));
}
