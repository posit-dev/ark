...
fn test_execute_request_multiple_expressions() {
    ...
    // We get output for all intermediate results, in both console and
    // notebook mode. Note that `print()` returns invisibly.
...
#[test]
fn test_execute_request_data_frame() {
    let frontend = DummyArkFrontend::lock();
    ...
    // Console mode: no inline data explorer MIME
