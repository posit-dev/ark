...
pub fn ui_frontend_reply_from_value(
    ...
    match request {
        UiFrontendRequest::ShowQuestion(_) => Ok(UiFrontendReply::ShowQuestionReply(
        ...
        UiFrontendRequest::WorkspaceFolder => Ok(UiFrontendReply::WorkspaceFolderReply(
            serde_json::from_value(reply)?,
        )),
        UiFrontendRequest::ModifyEditorSelections(_) => {
            Ok(UiFrontendReply::ModifyEditorSelectionsReply())
        },
        UiFrontendRequest::LastActiveEditorContext => Ok(
            UiFrontendReply::LastActiveEditorContextReply(serde_json::from_value(reply)?),
        ),
    }
}
