...
",
    ...
    assert_eq!(bindings.definitions().len(), 2);
    assert!(bindings.definitions().contains(&DefinitionId::from(1)));
    assert!(bindings.definitions().contains(&DefinitionId::from(2)));
    assert_not!(bindings.may_be_unbound());
}
...
",
    ...
    let bindings = map.bindings_at_use(UseId::from(1));
    assert_eq!(bindings.definitions().len(), 2);
    assert!(bindings.definitions().contains(&DefinitionId::from(0)));
    assert!(bindings.definitions().contains(&DefinitionId::from(1)));
    assert_not!(bindings.may_be_unbound());
}
...
",
    ...
    assert_eq!(bindings.definitions().len(), 3);
    assert!(bindings.definitions().contains(&DefinitionId::from(0)));
    assert!(bindings.definitions().contains(&DefinitionId::from(1)));
    assert!(bindings.definitions().contains(&DefinitionId::from(2)));
    assert_not!(bindings.may_be_unbound());
...
fn test_super_assignment_not_in_function_use_def() {
    let index = index(
        "\
function() {
    x <<- 1      # lands in file scope, not in function's use-def map
...
",
    ...
    assert_eq!(second_x.definitions().len(), 2);
    assert!(second_x.definitions().contains(&DefinitionId::from(1)));
    assert!(second_x.definitions().contains(&DefinitionId::from(2)));
...
",
    ...
    let second_x = map.bindings_at_use(UseId::from(4));
    assert_eq!(second_x.definitions().len(), 2);
    assert!(second_x.definitions().contains(&DefinitionId::from(0)));
    assert!(second_x.definitions().contains(&DefinitionId::from(1)));
    assert_not!(second_x.may_be_unbound());
}
