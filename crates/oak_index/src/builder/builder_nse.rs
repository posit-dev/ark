...
impl<'a> SemanticIndexBuilder<'a> {
    ...
    pub(super) fn resolve_nse_callee(&self, call: &RCall) -> Option<&'static NseAnnotation> {
        ...
        match &func {
            AnyRExpression::RIdentifier(ident) => {
                ...
                if !self.use_def_maps[self.current_scope].is_unbound(symbol_id) {
                    return None;
                }
    ...
    fn resolve_nse_name(&self, name: &str) -> Option<&'static NseAnnotation> {
        match self.nse_resolver.resolve(name) {
    ...
    pub(super) fn add_definition_to_parent(
        ...
        self.use_def_maps[parent_scope].ensure_symbol(symbol_id);
        ...
        self.use_def_maps[parent_scope].record_deferred_definition(symbol_id, def_id);
    }
}
