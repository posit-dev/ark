...
#[test]
fn set_file_is_idempotent_on_same_url() {
    let mut db = OakDatabase::new();
    let url = file_url("a.R");

    let first = db.set_file(url.clone(), "x <- 1\n".to_string(), None);
    let second = db.set_file(url.clone(), "x <- 2\n".to_string(), None);
...
#[test]
fn set_file_updates_package_on_re_intern() {
    ...
    let file = db.set_file(url.clone(), "x <- 1\n".to_string(), None);
