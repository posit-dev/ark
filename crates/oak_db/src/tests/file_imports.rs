...
/// A workspace of scripts at `ws`, returned with the root and in the order
/// given.
...
fn test_shiny_entry_sees_global_then_the_autoloaded_directory() {
    ...
    // LIFO: the `R/` files run last and into an inner environment, so they
    // shadow `global.R`.
...
fn test_shiny_ui_and_server_do_not_see_each_other() {
    // Shiny evaluates them into sibling environments, so neither is part of the
    // other's support set. Pinned so nobody later "fixes" it.
    let mut db = TestDb::new();
    let (_, files) = script_workspace(&mut db, &[
...
fn test_shiny_disable_autoload_drops_the_directory_but_keeps_global() {
    // The opt-out returns from `loadSupport()` after the `global.R` block.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let (_, files) = script_workspace(&mut db, &[
        ("ws/app.R", "shinyApp(ui, server)\n"),
    ...
    // `a.R` itself is never sourced by `loadSupport()` when disabled, so it
    // gets none of the Shiny treatment either: no inherited `global.R`, and
    // no implicit `shiny` attach. Plain `R/` collation applies instead --
    // `_disable_autoload.R` is a real file in the directory, so it shows up
    // as an ordinary (empty) collation sibling.
    assert_eq!(shape(&db, a.imports(&db)), vec![
...
fn test_confirmed_entry_point_with_nothing_to_inherit_still_attaches_shiny() {
    // `loadSupport()` calls `require(shiny)` unconditionally, before it even
    // checks whether `global.R` or `R/` exist. So a lone `app.R` with the
    // marker call has no `File` layer to inherit, but it's still a real Shiny
    // app and still gets the attach.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
...
fn test_autoloaded_file_with_no_global_still_gets_the_implicit_shiny_attach() {
    // `shiny_autoload` returns `Some(vec![])` here, not `None`: no `global.R`
    // to inherit from, but `loadSupport()` still ran `require(shiny)` first.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let (_, files) = script_workspace(&mut db, &[
        ("ws/app.R", "shinyApp(ui, server)\n"),
...
fn test_autoloaded_file_sees_global_and_the_implicit_shiny_attach() {
    // `loadSupport()` evaluates `global.R` first and opens with
    // `require(shiny)`, so a module file gets both without asking.
    let mut db = TestDb::new();
...
fn test_r_directory_without_an_entry_point_ignores_global() {
    // A `global.R` beside an `R/` only means something to Shiny, so without an
    // entry point the directory keeps the plain collation view.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let (_, files) = script_workspace(&mut db, &[
        ("ws/global.R", "cfg <- 1\n"),
...
fn test_app_r_without_the_shiny_app_call_is_not_detected() {
    // Same filename and layout as a real entry point, but the content marker
    // `getShinyFileType()` requires (`shinyApp` for `app.R`) is missing, so it
    // reads as a plain script that happens to be named `app.R`.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let (_, files) = script_workspace(&mut db, &[
        ("ws/app.R", "1 + 1\n"),
...
fn test_shiny_layers_backdate_on_unrelated_script_change() {
    // `shiny_autoload` reads every workspace root's scripts, so a script added
    // anywhere re-executes it. Filtered to the app directory the answer is
    // unchanged, so salsa backdates it and `cross_file_layers` stands. Reading
    // the scripts straight from `cross_file_layers` would lose that.
    let mut db = TestDb::new();
...
fn test_editing_entry_file_body_reexecutes_only_the_classifier() {
    // `is_shiny_entry_file` is its own tracked query specifically so its
    // content read backdates independently. Without it, every `R/` file in
    // the app would reach `app.R`'s `source_text` through `is_shiny_dir`, and
    // any edit there -- marker-preserving or not -- would re-execute their
    // `shiny_autoload`. With it, only the classifier re-runs; its boolean is
    // unchanged, so `shiny_autoload` and `cross_file_layers` stay cached.
    let mut db = TestDb::new();
    let (_, files) = script_workspace(&mut db, &[
        ("ws/app.R", "shinyApp(ui, server)\n"),
        ("ws/R/a.R", "a_val <- 1\n"),
    ...
    // A comment above the call: `shinyApp` stays in the text.
...
fn test_removing_the_shiny_app_call_drops_the_shiny_layers() {
    // The other side of the firewall: this is a real invalidation, not
    // churn. Once `app.R`'s content no longer classifies it as an entry
    // point, both the app and its `R/` sibling lose the `shiny` layer.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let (_, files) = script_workspace(&mut db, &[
        ("ws/app.R", "shinyApp(ui, server)\n"),
...
fn test_adding_the_shiny_app_call_creates_the_shiny_layers() {
    // The remaining direction: a plain script named `app.R` picks up the
    // layers the moment an edit gives it the marker call, on both itself and
    // its `R/` sibling.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let (_, files) = script_workspace(&mut db, &[
...
#[test]
fn test_package_contained_shiny_app_is_out_of_scope() {
    // golem-style `inst/app/app.R`: a real Shiny app, but owned by a package.
    // `cross_file_layers`'s Shiny arms are gated on `package(db) == None`, so
    // a file with a package back-pointer never reaches them, regardless of
    // its own name or content -- this stays standalone rather than crashing
    // or guessing. Explicitly out of scope for now: even lifting the `None`
    // gate wouldn't be enough, since `files_in_directory` only lists
    // `workspace_scripts`, never `Package::scripts`, so `app.R`'s own `R/`
    // siblings under `inst/app/` wouldn't be found either. See the "Shiny-in-
    // a-package" gap in the design note.
    let mut db = TestDb::new();
    install_packages(&mut db, &["base", "shiny"]);
    let workspace = workspace_root(&db, "w");
    let pkg = Package::new(
