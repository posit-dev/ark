...
fn test_file_by_url_finds_workspace_script() {
    ...
    assert_eq!(db.file_by_url(&file_url("proj/a.R")), Some(file));
}
...
fn test_file_by_url_finds_workspace_package_file() {
    ...
    assert_eq!(db.file_by_url(&file_url("proj/R/foo.R")), Some(file));
}

#[test]
fn test_file_by_url_finds_library_package_file() {
    let mut db = TestDb::new();
    ...
    assert_eq!(db.file_by_url(&file_url("libs/foo/R/a.R")), Some(file));
}
...
#[test]
fn test_file_by_url_returns_none_when_absent() {
    let db = TestDb::new();
    assert_eq!(db.file_by_url(&file_url("missing.R")), None);
}

#[test]
fn test_root_url_index_invalidates_per_root() {
    ...
    // by counting salsa `WillExecute` events on `root_url_index`.
    let mut db = TestDb::new();
    ...
    let _ = db.file_by_url(&file_url("a/file.R"));
    let _ = db.file_by_url(&file_url("b/file.R"));
    ...
    let _ = db.file_by_url(&file_url("b/file.R"));
    assert_eq!(db.executions("root_url_index"), 3);
}
